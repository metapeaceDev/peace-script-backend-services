...
async def init_db():
    ...
    try:
        if settings.USE_MOCK_DB:
            logger.warning("🔸 USING MOCK DATABASE (IN-MEMORY) - Data will be lost on restart")
        ...
        # Build a mock client as fallback if strict mode is off, otherwise re-raise
        if settings.DEBUG_MODE:
            logger.warning("Fallback to Mock DB due to connection failure.")
            client = AsyncMongoMockClient()
            database = client.get_database(settings.MONGO_DB_NAME)
        else:
            raise e
