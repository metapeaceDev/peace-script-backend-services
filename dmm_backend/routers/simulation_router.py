...
async def run_simulation(
    ...
    🔒 Protected: Requires authentication OR character mode
    ...
    Authentication:
    - User ID extracted from JWT token (optional if character_id provided)
    ...
    if is_character_mode:
        ...
        if not current_user:
            raise HTTPException(
                status_code=401,
                detail="Authentication required for user mode simulation"
            )
