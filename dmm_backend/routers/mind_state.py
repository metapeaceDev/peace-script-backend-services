...
# Optional auth dependency for demo access
...
async def get_mind_state(
    user_id: str,
    current_user: Optional[User] = Depends(optional_current_active_user)
):
    """
    Get MindState for a specific user
    
    **Authentication required** unless user_id is "demo_user"
    ...
    if user_id == "demo_user":
        pass
    # Check ownership
    elif not current_user or (user_id != str(current_user.id) and not current_user.is_superuser):
        raise HTTPException(
            status_code=403,
